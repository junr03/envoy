...
private:
  using ListenerList = std::list<Network::ListenerConfigPtr>;

  bool addOrUpdateListenerInternal(const envoy::api::v2::Listener& config,
                                   const std::string& version_info, bool added_via_api,
                                   const std::string& name);
  struct DrainingListener {
    DrainingListener(Network::ListenerConfigPtr&& listener, uint64_t workers_pending_removal)
        : listener_(std::move(listener)), workers_pending_removal_(workers_pending_removal) {}

    Network::ListenerConfigPtr listener_;
    uint64_t workers_pending_removal_;
  };

  void addListenerToWorker(Worker& worker, Network::ListenerConfig& listener);
   ...
   */
  void drainListener(Network::ListenerConfigPtr&& listener);
